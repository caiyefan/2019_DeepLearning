...
class NN_Model():
    ...
    def add_layer(self, input_nodes, output_nodes, activation_type="sigmoid", train_activation=False):
        ...
        self.activation_c.append(np.ones((1, output_nodes)))
    ...
    def forward(self, input):
        ...
        for i in range(self.num_layer):
            ...
            self.X[i + 1] = activation_function(self.Z[i + 1], self.activation_c[i + 1],
        ...
        for i in range(self.num_layer):
            ...
            if j == self.num_layer:
                if self.activation_type[j] == "softmax":
                    ...
                    self.error_layer[j] = loss_function_d(target, self.X[j], type=loss_type)
                    self.delta_layer[j] = self.error_layer[j] * activation_function_d(self.X[j], self.activation_c[j],
    ...
    def update(self, lr=0.01):
        ...
        for i in range(self.num_layer):
            ...
            if self.activation_type[j] == "sigmoid" and self.train_activation[j]:
                self.activation_c[j] += np.sum(sigmoid(self.Z[j], 1) * self.error_layer[j], axis=0) * lr
    ...
    def train_dog(self, input, target, lr=0.05, loss_type="cross entropy"):
        right = 0.0
        loss_sum = 0
        r = 0
    ...
    def draw(self):
        ...
        plt.title("Learning Result - Loss")
        plt.xlabel("Epochs")
        plt.ylabel("Loss")

        plt.subplot(212)
        plt.plot(self.acc_list, label="Accuracy")
        plt.title("Learning Result - Accuracy")
        plt.xlabel("Epochs")
        plt.ylabel("Accuracy")
    ...
    def check(self, weights=None, bias=None, activation=None):
        ...
        if activation:
            ...
            print("Activation_c: ", end='')
            print(*self.activation_c)
